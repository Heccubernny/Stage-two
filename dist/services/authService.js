...
class AuthService {
    static register(_a) {
        return __awaiter(this, arguments, void 0, function* ({ firstName, lastName, email, password, phone, }) {
            ...
            const saltLength = 10;
            const doesEmailExist = yield userRespository.exists({ where: { email } });
            if (doesEmailExist) {
            ...
            const token = jsonwebtoken_1.default.sign({ userId: user.userId }, process.env.HNG_JWT_SECRET, { expiresIn: process.env.EXPIRES_IN });
            return {
                token,
                user: {
                    userId: user.userId,
    ...
    static login(_a) {
        return __awaiter(this, arguments, void 0, function* ({ email, password }) {
            ...
            const token = jsonwebtoken_1.default.sign({ userId: user.userId }, process.env.HNG_JWT_SECRET, {
